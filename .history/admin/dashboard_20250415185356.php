...
$stmt_avg = $pdo->query("SELECT AVG(amount) as average FROM contributions");
$avg_contribution = $stmt_avg->fetch()['average'] ?? 0;

// Total contributors
$stmt_contributors = $pdo->query("SELECT COUNT(DISTINCT user_id) as total FROM contributions");
$total_contributors = $stmt_contributors->fetch()['total'] ?? 0;

// Top 5 contributors
$stmt_top = $pdo->query("SELECT users.name, SUM(contributions.amount) as total 
                      FROM contributions 
...
$stmt_recent = $pdo->query("SELECT users.name, contributions.amount, contributions.contributed_at 
                       FROM contributions 
                       JOIN users ON users.id = contributions.user_id 
...
$tree_stmt = $pdo->query("SELECT users.name, users.email, users.id, users.referred_by, COALESCE(SUM(contributions.amount),0) as total_contribution FROM users LEFT JOIN contributions ON users.id = contributions.user_id GROUP BY users.id");
    ...
    <aside id="sidebar" class="bg-indigo-800 text-white w-64 min-h-screen fixed z-10 transition-transform duration-300 ease-in-out md:translate-x-0 transform -translate-x-full">
      <div class="p-4">
        ...
        <nav>
          <ul class="space-y-2">
            <li>
              <a href="dashboard.php" class="flex items-center p-2 rounded bg-indigo-900 hover:bg-indigo-900">
    ...
    <main class="flex-1 p-6 md:ml-64 transition-all duration-300 ease-in-out overflow-x-hidden">
        ...
        <div class="bg-white rounded-lg shadow p-6">
          <div class="flex items-center">
            ...
            <div>
              ...
              <p class="text-2xl font-bold"><?php echo count(array_filter($users, function($u) { return !$u['referred_by']; })); ?></p>
            </div>
          </div>
        </div>
      </div>
          ...
          <table class="w-full table-auto">
            ...
            <tbody>
              ...
              <tr class="border-b hover:bg-gray-50">
                ...
                <td class="p-3 font-semibold text-green-600">$<?php echo number_format($contribution['amount'], 2); ?></td>
                <td class="p-3"><?php echo date('M j, Y g:i A', strtotime($contribution['contributed_at'])); ?></td>
              </tr>
              <?php endforeach; ?>
            </tbody>
          </table>
      ...
      <div class="bg-white rounded-lg shadow p-6 mb-6">
        <div class="flex justify-between items-center mb-4">
          ...
          <div>
            <input type="text" id="userSearch" placeholder="Search users..." class="px-4 py-2 border rounded">
          </div>
        </div>
        <div class="overflow-x-auto">
          <table class="w-full table-auto" id="userTable">
            ...
            <tbody>
              ...
              <tr class="border-b hover:bg-gray-50">
                ...
                <td class="p-3 font-semibold text-green-600">$<?php echo number_format($user['total_contribution'] ?? 0, 2); ?></td>
                <td class="p-3">
                  <a href="user_detail.php?id=<?php echo $user['id']; ?>" class="text-indigo-600 hover:text-indigo-800">
  ...
  <div class="bg-white rounded-lg shadow p-6">
    ...
    <ul class="divide-y">
      ...
      <li class="py-3 flex justify-between items-center">
        ...
        <span class="text-green-600 font-bold">$<?php echo number_format($contributor['total'], 2); ?></span>
      </li>
      <?php endforeach; ?>
    </ul>
  </div>
