...
try {
    ...
    $stmt = $pdo->prepare("
        ...
        SELECT id, name, level FROM RefTree
        WHERE id != ?
        ORDER BY level, name
    ");
    ...
    // Date filter
    $date_filter = match ($filter_period) {
        '7days' => "AND c.created_at >= DATE_SUB(CURRENT_DATE(), INTERVAL 7 DAY)",
        '30days' => "AND c.created_at >= DATE_SUB(CURRENT_DATE(), INTERVAL 30 DAY)",
        '90days' => "AND c.created_at >= DATE_SUB(CURRENT_DATE(), INTERVAL 90 DAY)",
        'year' => "AND c.created_at >= DATE_SUB(CURRENT_DATE(), INTERVAL 1 YEAR)",
        default => '',
    };

    // SQL filters
    $user_filter = ($filter_user !== 'all') ? "AND c.user_id = :filter_user" : '';
    $project_filter = ($filter_project !== 'all') ? "AND c.project_id = :filter_project" : '';
    ...
    $count_sql = "
        ...
        WHERE c.user_id != :user_id $date_filter $user_filter $project_filter
    ";

    $params = ['user_id' => $user_id];
    if ($filter_user !== 'all') $params['filter_user'] = $filter_user;
    if ($filter_project !== 'all') $params['filter_project'] = $filter_project;
    ...
    // Bind all parameters including offset and limit
    ...
    // You can now use $contributions, $downline_users, $projects, $referral_link, etc. in your output or view.
