...
<body class="bg-gray-100 text-gray-800">
                ...
                <div class="flex justify-between items-center mb-3">
                    <div>
                        <button id="refresh-tree"
                            ...
                            <i class="fas fa-sync-alt mr-1"></i> Refresh Tree
                        </button>
                    </div>
                    <div id="loading-indicator" class="hidden text-indigo-600 text-sm">
    ...
    function buildHierarchy(data, rootId) {
        ...
        dataCopy.forEach(item => {
            // Create clean node object with only needed properties
            map[item.id] = {
                id: item.id,
                name: item.name,
                contribution: item.contributions || 0,
                treeContribution: item.tree_contribution || 0,
                childrenCount: 0, // Will be calculated below
                level: item.level || 0,
                children: []
            };
        });
        ...
        dataCopy.forEach(item => {
            if (item.id !== rootId && item.referred_by && map[item.referred_by] && map[item.id]) {
                map[item.referred_by].children.push(map[item.id]);
            }
        });

        // Calculate childrenCount for each node
    ...
    function renderTree() {
        ...
        node.append("text")
            ...
            .text(d => {
                // Get the correct name for this node
