...
// Get all projects for the project filter
...
$filter_project = $_GET['project_id'] ?? 'all'; // New project filter
...
if ($filter_project !== 'all') {
    $project_filter = "AND c.project_id = " . intval($filter_project);
}
...
$stmt = $pdo->prepare("WITH RECURSIVE RefTree AS (
                      ...
                      WHERE c.user_id != ? $date_filter $user_filter $project_filter");
$stmt->execute([$user_id, $user_id]);
...
$stmt = $pdo->prepare("WITH RECURSIVE RefTree AS (
                      ...
                      FROM contributions c
                      JOIN RefTree rt ON c.user_id = rt.id
                      WHERE c.user_id != ? $date_filter $project_filter
                      GROUP BY rt.level
                      ORDER BY rt.level");
$stmt->execute([$user_id, $user_id]);
...
$stmt = $pdo->prepare("WITH RECURSIVE RefTree AS (
                      ...
                      WHERE (c.id IS NULL OR 1=1 $project_filter)
                      GROUP BY d.date
                      ORDER BY d.date");
$stmt->execute([$user_id, $user_id]);
...
$stmt = $pdo->prepare("WITH RECURSIVE RefTree AS (
                      ...
                      ORDER BY total_amount DESC");
$stmt->execute([$user_id, $user_id]);
$project_distribution = $stmt->fetchAll();
    ...
    <div class="flex pt-16">
        ...
        <main class="flex-1 md:ml-64 p-6">
            <div class="mb-8">
                ...
                <!-- Add this in the filter form, after the user_id dropdown -->
                <div>
