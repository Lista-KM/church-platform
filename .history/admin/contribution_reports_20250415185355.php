    ...
    <main class="flex-1 p-6 md:ml-64 transition-all duration-300 ease-in-out">
      ...
      <div class="bg-white rounded-lg shadow p-6 mb-6">
        ...
        <form action="" method="GET" class="flex flex-col md:flex-row md:items-end gap-4">
          <div class="flex-1">
            ...
            <input type="text" id="start_date" name="start_date" class="datepicker w-full px-4 py-2 border rounded" value="<?php echo $start_date; ?>">
          </div>
          <div class="flex-1">
            <label for="end_date" class="block text-sm font-medium text-gray-700 mb-1">End Date</label>
            <input type="text" id="end_date" name="end_date" class="datepicker w-full px-4 py-2 border rounded" value="<?php echo $end_date; ?>">
          </div>
          <div>
            <button type="submit" class="bg-indigo-600 text-white px-4 py-2 rounded hover:bg-indigo-700">Apply Filter</button>
          </div>
        </form>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        <div class="bg-white rounded-lg shadow p-4">
          ...
          <p class="text-2xl font-bold text-indigo-600">$<?php echo number_format($summary['total'] ?? 0, 2); ?></p>
        </div>
        <div class="bg-white rounded-lg shadow p-4">
          <h4 class="text-gray-500 text-sm mb-1">Number of Contributions</h4>
          <p class="text-2xl font-bold text-indigo-600"><?php echo number_format($summary['count'] ?? 0); ?></p>
        </div>
        <div class="bg-white rounded-lg shadow p-4">
          <h4 class="text-gray-500 text-sm mb-1">Average Contribution</h4>
          <p class="text-2xl font-bold text-indigo-600">$<?php echo number_format($summary['average'] ?? 0, 2); ?></p>
        </div>
        <div class="bg-white rounded-lg shadow p-4">
          <h4 class="text-gray-500 text-sm mb-1">Largest Contribution</h4>
          <p class="text-2xl font-bold text-indigo-600">$<?php echo number_format($summary['highest'] ?? 0, 2); ?></p>
        </div>
        <div class="bg-white rounded-lg shadow p-4">
          <h4 class="text-gray-500 text-sm mb-1">Smallest Contribution</h4>
          <p class="text-2xl font-bold text-indigo-600">$<?php echo number_format($summary['lowest'] ?? 0, 2); ?></p>
        </div>
      </div>
          ...
          <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr>
                  <td class="px-6 py-4 whitespace-nowrap"><?php echo htmlspecialchars($contributor['name']); ?></td>
                  <td class="px-6 py-4 whitespace-nowrap font-medium text-green-600">$<?php echo number_format($contributor['total'], 2); ?></td>
                  <td class="px-6 py-4 whitespace-nowrap"><?php echo $contributor['frequency']; ?> times</td>
                  <td class="px-6 py-4 whitespace-nowrap"><?php echo date('M j, Y', strtotime($contributor['last_contribution'])); ?></td>
                </tr>
                <?php endforeach; ?>
              </tbody>
            </table>
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              <tr>
                <td class="px-6 py-4 whitespace-nowrap"><?php echo htmlspecialchars($contribution['name']); ?></td>
                <td class="px-6 py-4 whitespace-nowrap font-medium text-green-600">$<?php echo number_format($contribution['amount'], 2); ?></td>
                <td class="px-6 py-4 whitespace-nowrap"><?php echo date('M j, Y g:i A', strtotime($contribution['contributed_at'])); ?></td>
                <td class="px-6 py-4 whitespace-nowrap"><?php echo htmlspecialchars($contribution['payment_method'] ?? 'N/A'); ?></td>
              </tr>
              <?php endforeach; ?>
            </tbody>
          </table>
      ...
      <div class="bg-white rounded-lg shadow p-6">
        ...
        <div class="flex flex-wrap gap-4">
          <a href="export_report.php?type=csv&start=<?php echo $start_date; ?>&end=<?php echo $end_date; ?>" class="bg-gray-600 text-white px-4 py-2 rounded hover:bg-gray-700 flex items-center">
            <i class="fas fa-file-csv mr-2"></i> Export CSV
          </a>
          <a href="export_report.php?type=excel&start=<?php echo $start_date; ?>&end=<?php echo $end_date; ?>" class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 flex items-center">
            <i class="fas fa-file-excel mr-2"></i> Export Excel
          </a>
          <a href="export_report.php?type=pdf&start=<?php echo $start_date; ?>&end=<?php echo $end_date; ?>" class="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700 flex items-center">
      ...
      data: {
        labels: [
          <?php foreach ($size_distribution as $data): ?>
            "<?php echo $data['range_label']; ?> (<?php echo $data['count']; ?>)",
          <?php endforeach; ?>
        ],
        datasets: [{
          data: [
