...
<body class="bg-gray-100 text-gray-800">
    ...
    function buildHierarchy(data, rootId) {
        ...
        data.forEach(item => {
            if (item.id !== rootId && item.referred_by && map[item.referred_by]) {
                map[item.referred_by].children.push(map[item.id]);
                map[item.referred_by].childrenCount++;
            }
        });
    ...
    function renderTree() {
        ...
        const node = g.selectAll(".node")
            ...
            .on("mouseover", function(event, d) {
                ...
                Contribution: $${parseFloat(d.data.contribution).toFixed(2)}<br>
                Team Contribution: $${parseFloat(d.data.treeContribution || d.data.contribution).toFixed(2)}
            `)
