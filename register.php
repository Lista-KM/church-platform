...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Check if user already exists with this email (might have contributed before)
    $checkUser = $pdo->prepare("SELECT id FROM users WHERE email = ?");
    $checkUser->execute([$email]);
    $existingUser = $checkUser->fetch();
    
    if ($existingUser) {
        // Update existing user with password to "activate" the account
        $stmt = $pdo->prepare("UPDATE users SET password = ?, referred_by = COALESCE(referred_by, ?) WHERE id = ?");
        $stmt->execute([$password, $referralId, $existingUser['id']]);
        $lastInsertId = $existingUser['id'];
        
        // Make sure they have a referral code
        $checkCode = $pdo->prepare("SELECT referral_code FROM users WHERE id = ?");
        $checkCode->execute([$lastInsertId]);
        $hasCode = $checkCode->fetchColumn();
...
<body class="bg-gray-100 min-h-screen flex justify-center items-center">
    <div class="w-full max-w-md bg-white p-8 rounded-lg shadow-md">
        ...
        <form method="POST">
            ...
            <button type="submit"
                class="w-full py-2 px-4 bg-indigo-600 text-white font-semibold rounded-lg hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 class="
                w-full py-2 px-4 bg-indigo-600 text-white font-semibold rounded-lg hover:bg-indigo-700
