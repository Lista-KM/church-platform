...
try {
    ...
    $stmt = $pdo->prepare("SELECT COALESCE(SUM(amount), 0) as personal_total FROM contributions WHERE user_id = ?");
    ...
    // Get daily contribution data with proper SQL injection protection
    // Get daily contribution data with proper SQL injection protection
$daily_sql = "
WITH RECURSIVE RefTree AS (
    SELECT id FROM users WHERE id = :user_id
...
FROM date_range d
LEFT JOIN contributions c ON DATE(c.created_at) = d.date AND c.user_id IN (SELECT id FROM RefTree WHERE id != :user_id)";

// This is the corrected part - use AND instead of WHERE for the additional condition
if ($filter_project !== 'all') {
    $daily_sql .= " AND c.project_id = :filter_project";
    $params['filter_project'] = $filter_project;
}

$daily_sql .= " GROUP BY d.date ORDER BY d.date";
                    ...
                    <div class="bg-white p-6 rounded-lg shadow-md mb-6">
                        ...
                        <div class="flex flex-col md:flex-row items-start md:items-center gap-4">
                            <div class="flex-1">
                                ...
                                <p class="text-sm text-gray-500 mt-2">Share this link with friends to earn rewards on
                                    their
                                    contributions</p>
                            </div>
