...
$referral_url = $base_url . '/register.php?ref=' . urlencode($user['referral_code']);
...
<body class="bg-gray-100 text-gray-800">
  ...
  <header class="bg-indigo-700 text-white p-4 flex justify-between items-center shadow fixed w-full z-10">
    ...
    <div class="flex items-center space-x-4">
      ...
      <a href="logout.php" class="bg-white text-indigo-700 px-3 py-1 rounded hover:bg-gray-100 transition text-sm md:px-4 md:py-2 md:text-base">Logout</a>
    </div>
  </header>

  <div class="flex pt-16">
    <!-- Sidebar -->
    <aside id="sidebar" class="bg-indigo-800 text-white w-64 min-h-screen fixed z-20 transition-transform duration-300 ease-in-out transform -translate-x-full md:translate-x-0 overflow-y-auto">
      <div class="p-4">
        ...
        <nav>
          <ul class="space-y-2">
            <li>
              <a href="user_dashboard.php" class="flex items-center p-2 rounded hover:bg-indigo-700 transition">
            ...
            <li>
              <a href="contributions.php" class="flex items-center p-2 rounded hover:bg-indigo-700 transition">
  ...
  <div class="mb-8">
    ...
    <div class="bg-white p-6 rounded-lg shadow-lg mt-6">
      ...
      <form action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_blank" class="space-y-4">
        ...
        <div>
          <label for="paypal_amount" class="block text-sm font-medium text-gray-600">Amount in USD</label>
          <input type="number" name="amount" id="paypal_amount" placeholder="Enter amount in USD" class="mt-2 block w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" required>
        </div>
        <button type="submit" class="w-full py-2 px-4 bg-indigo-700 text-white rounded-lg hover:bg-indigo-800 focus:outline-none focus:ring-2 focus:ring-indigo-500">Pay with PayPal</button>
      </form>
    </div>
          ...
          <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
            <div class="flex justify-between items-start">
              <div>
                ...
                <p class="text-2xl md:text-3xl font-bold">$<?php echo number_format($personal_stats['total_amount'] ?? 0, 2); ?></p>
              </div>
          ...
          <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
            <div class="flex justify-between items-start">
              <div>
                ...
                <p class="text-2xl md:text-3xl font-bold">$<?php echo number_format($downline_stats['total_downline_contributions'] ?? 0, 2); ?></p>
              </div>
          ...
          <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
            <div class="flex justify-between items-start">
              <div>
                ...
                <p class="text-2xl md:text-3xl font-bold">$<?php echo number_format(($personal_stats['total_amount'] ?? 0) + ($downline_stats['total_downline_contributions'] ?? 0), 2); ?></p>
              </div>
          ...
          <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
            <div class="flex justify-between items-start">
              <div>
                ...
                <p class="text-2xl md:text-3xl font-bold"><?php echo number_format($referral_stats['total_referrals'] ?? 0); ?></p>
              </div>
        ...
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md mb-8">
          <div class="flex justify-between items-center mb-4">
            <h3 class="text-lg font-semibold">Referral Tree</h3>
            <div>
              <button id="refresh-tree" class="text-indigo-600 hover:text-indigo-800 focus:outline-none text-sm md:text-base">
            ...
            <table class="min-w-full bg-white">
              ...
              <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                  <td class="py-2 px-3 md:py-3 md:px-4 text-xs md:text-sm"><?php echo htmlspecialchars($referral['name']); ?></td>
                  <td class="py-2 px-3 md:py-3 md:px-4 text-xs md:text-sm"><?php echo date('M j, Y', strtotime($referral['created_at'])); ?></td>
                  <td class="py-2 px-3 md:py-3 md:px-4 text-right text-xs md:text-sm"><?php echo number_format($referral['their_referrals']); ?></td>
                  <td class="py-2 px-3 md:py-3 md:px-4 text-right text-xs md:text-sm">$<?php echo number_format($referral['contributions'] ?? 0, 2); ?></td>
 ...
 <!-- Referral Link Generator -->
    <div class="bg-white p-4 md:p-6 rounded-lg shadow-md mb-8">
      ...
      <p class="mb-4 text-gray-600">Share your unique invite link with friends and family to grow your network.</p>

      <div class="mb-6">
        <div class="flex flex-col md:flex-row md:items-center md:space-x-2 mb-2">
          <input id="referral-link" type="text" value="<?php echo htmlspecialchars($referral_url); ?>" class="w-full p-3 border border-gray-300 rounded-lg bg-gray-50 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 mb-2 md:mb-0">
          <button id="copy-link" class="bg-indigo-600 text-white px-4 py-3 rounded-lg hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition whitespace-nowrap">
            <i class="fas fa-copy mr-2"></i> Copy
          </button>
        </div>
        <div class="text-sm text-gray-500">Your Invite code: <span class="font-mono font-bold"><?php echo htmlspecialchars($user['referral_code']); ?></span></div>
      </div>

      <div>
        <h4 class="text-md font-medium mb-3">Share directly:</h4>
        <div class="flex flex-wrap gap-3">
          <a href="https://wa.me/?text=<?php echo urlencode('Join me on XYZ Platform! Use my referral link: ' . $referral_url); ?>" target="_blank" class="bg-green-500 text-white px-3 py-2 rounded-lg hover:bg-green-600 transition flex items-center text-sm">
            <i class="fab fa-whatsapp mr-2"></i> WhatsApp
          </a>
          <a href="https://www.facebook.com/sharer/sharer.php?u=<?php echo urlencode($referral_url); ?>" target="_blank" class="bg-blue-600 text-white px-3 py-2 rounded-lg hover:bg-blue-700 transition flex items-center text-sm">
            <i class="fab fa-facebook-f mr-2"></i> Facebook
          </a>
          <a href="https://twitter.com/intent/tweet?text=<?php echo urlencode('Join me on XYZ Platform! Use my referral link: ' . $referral_url); ?>" target="_blank" class="bg-blue-400 text-white px-3 py-2 rounded-lg hover:bg-blue-500 transition flex items-center text-sm">
            <i class="fab fa-twitter mr-2"></i> Twitter
          </a>
          <a href="mailto:?subject=<?php echo urlencode('Join me on XYZ Platform'); ?>&body=<?php echo urlencode('Hey! I thought you might be interested in XYZ Platform. Use my referral link to sign up: ' . $referral_url); ?>" class="bg-gray-600 text-white px-3 py-2 rounded-lg hover:bg-gray-700 transition flex items-center text-sm">
            <i class="fas fa-envelope mr-2"></i> Email
          </a>
          <a href="sms:?body=<?php echo urlencode('Join me on XYZ Platform! Use my referral link: ' . $referral_url); ?>" class="bg-purple-600 text-white px-3 py-2 rounded-lg hover:bg-purple-700 transition flex items-center text-sm">
          ...
          <div class="overflow-x-auto -mx-4 md:mx-0">
            <table class="min-w-full bg-white">
              ...
              <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                  <td class="py-2 px-3 md:py-3 md:px-4 text-xs md:text-sm"><?php echo htmlspecialchars($contribution['name']); ?></td>
                  <td class="py-2 px-3 md:py-3 md:px-4 text-xs md:text-sm"><?php echo htmlspecialchars($contribution['referred_by']); ?></td>
                  <td class="py-2 px-3 md:py-3 md:px-4 text-right text-xs md:text-sm">$<?php echo number_format($contribution['amount'], 2); ?></td>
                  <td class="py-2 px-3 md:py-3 md:px-4 text-right text-xs md:text-sm"><?php echo date('M j, Y', strtotime($contribution['created_at'])); ?></td>
                </tr>
                <?php endforeach; ?>
              </tbody>
            </table>
    ...
    function renderTree() {
      ...
      node.append("circle")
        ...
        .attr("fill", d => {
          ...
          if (d.data.treeContribution > 1000 || d.data.contribution > 1000) return "#10B981"; // High contribution
