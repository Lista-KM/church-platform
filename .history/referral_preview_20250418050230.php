        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="mb-8">
                ...
                <div class="flex items-center">
                    <span
                        class="bg-indigo-100 text-indigo-800 font-mono text-lg px-4 py-2 rounded"><?php echo htmlspecialchars($user['referral_code']); ?></span>
                    <button id="copy-btn" class="ml-3 text-indigo-600 hover:text-indigo-800"
                        onclick="copyReferralCode()">Copy</button>
                </div>
                <p class="mt-2 text-gray-600">Share your referral link: <span
                        class="text-gray-800"><?php echo htmlspecialchars($_SERVER['HTTP_HOST']); ?>/landing.php?ref=<?php echo htmlspecialchars($user['referral_code']); ?></span>
                </p>
            </div>
            ...
            <div class="mt-8">
                ...
                <div class="p-4 bg-gray-50 rounded-lg">
                    <?php echo displayFullReferralTree($pdo, $userId); ?>
                </div>
            </div>
        </div>
    </div>
...
<?php
function displayFullReferralTree($pdo, $userId, $level = 0) {
    $output = '';
    ...
    // Add children
    if ($referralCount > 0) {
        $output .= '<div class="ml-8 pl-4 border-l border-gray-300 mt-1">';
        foreach ($referrals as $referral) {
            $output .= displayFullReferralTree($pdo, $referral['id'], $level + 1);
        }
        $output .= '</div>';
    }
