...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'delete_user') {
        ...
        if ($has_referrals) {
            ...
            // Delete user's contributions first (foreign key constraint)
...
<body class="bg-gray-100 text-gray-800">
  ...
  <header class="bg-indigo-700 text-white p-4 flex justify-between items-center shadow fixed w-full z-10">
    <div class="flex items-center">
      ...
      <h1 class="text-xl font-semibold">XYZ Admin Dashboard</h1>
    </div>
  ...
  <div class="flex pt-16">
    ...
    <aside id="sidebar" class="bg-indigo-800 text-white w-64 min-h-screen fixed z-10 transition-transform duration-300 ease-in-out md:translate-x-0 transform -translate-x-full">
      <div class="p-4">
        <div class="border-b border-indigo-700 pb-4 mb-6">
        ...
        <nav>
          <ul class="space-y-2">
            ...
            <li>
              <a href="contribution_reports.php" class="flex items-center p-2 rounded hover:bg-indigo-900">
    ...
    <main class="flex-1 p-6 md:ml-64 transition-all duration-300 ease-in-out">
      ...
      <div class="flex flex-col md:flex-row md:justify-between md:items-center mb-6">
        ...
        <div class="flex flex-col md:flex-row gap-4">
          <button id="addUserBtn" class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 transition flex items-center">
            <i class="fas fa-plus mr-2"></i> Add New User
          </button>
          <div class="relative">
            <input type="text" id="userSearch" placeholder="Search users..." class="pl-10 pr-4 py-2 border rounded w-full">
          ...
          <table class="min-w-full divide-y divide-gray-200" id="usersTable">
            <thead class="bg-indigo-600 text-white">
              <tr>
                ...
                <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">Contributions</th>
                <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">Referrals</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              <tr class="hover:bg-gray-50">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full <?php echo $user['is_admin'] ? 'bg-indigo-100 text-indigo-800' : 'bg-gray-100 text-gray-800'; ?>">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-center">
                  <button class="text-indigo-600 hover:text-indigo-900 mx-1 editBtn" data-id="<?php echo $user['id']; ?>" data-name="<?php echo htmlspecialchars($user['name']); ?>" data-email="<?php echo htmlspecialchars($user['email']); ?>">
                    <i class="fas fa-edit"></i>
                  </button>
                  <button class="text-blue-600 hover:text-blue-900 mx-1 roleBtn" data-id="<?php echo $user['id']; ?>" data-is-admin="<?php echo $user['is_admin']; ?>">
                    <i class="fas fa-user-shield"></i>
                  </button>
                  <button class="text-red-600 hover:text-red-900 mx-1 deleteBtn" data-id="<?php echo $user['id']; ?>" data-name="<?php echo htmlspecialchars($user['name']); ?>">
    ...
    <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
      ...
      <form action="process_add_user.php" method="POST">
        <div class="mb-4">
          ...
          <input type="text" id="name" name="name" class="w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-indigo-500" required>
        </div>
        <div class="mb-4">
          <label for="email" class="block text-gray-700 font-medium mb-2">Email</label>
          <input type="email" id="email" name="email" class="w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-indigo-500" required>
        </div>
        <div class="mb-4">
          <label for="password" class="block text-gray-700 font-medium mb-2">Password</label>
          <input type="password" id="password" name="password" class="w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-indigo-500" required>
        </div>
        <div class="mb-4">
        ...
        <div class="flex justify-end">
          <button type="button" class="bg-gray-300 text-gray-800 px-4 py-2 rounded mr-2 closeModal">Cancel</button>
          <button type="submit" class="bg-indigo-600 text-white px-4 py-2 rounded hover:bg-indigo-700">Add User</button>
        </div>
      </form>
    </div>
  </div>
    ...
    <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
      ...
      <form method="POST">
        ...
        <div class="mb-4">
          ...
          <input type="text" id="edit_name" name="name" class="w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-indigo-500" required>
        </div>
        <div class="mb-4">
          <label for="edit_email" class="block text-gray-700 font-medium mb-2">Email</label>
          <input type="email" id="edit_email" name="email" class="w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-indigo-500" required>
        </div>
        <div class="flex justify-end">
          <button type="button" class="bg-gray-300 text-gray-800 px-4 py-2 rounded mr-2 closeModal">Cancel</button>
  ...
  <!-- Change Role Modal -->
  <div id="roleUserModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50 hidden">
    <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
      <div class="flex justify-between items-center mb-4">
  ...
  <div id="deleteUserModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50 hidden">
    <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
      ...
      <p class="mb-4">Are you sure you want to delete <span id="delete_user_name" class="font-semibold"></span>? This action cannot be undone.</p>
      <form method="POST">
