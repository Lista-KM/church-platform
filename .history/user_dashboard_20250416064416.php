...
<body class="bg-gray-100 text-gray-800">
    ...
    function renderTree() {
        ...
        // Use the hierarchical data directly - treeData should already be properly structured
        ...
        const node = g.selectAll(".node")
            ...
            .on("mouseover", function(event, d) {
                ...
                tooltip
                  ...
                  Direct Referrals: ${d.data.children ? d.data.children.length : 0}<br>
                  Contribution: $${(d.data.contributions || 0).toFixed(2)}<br>
                  Team Contribution: $${(calculateTeamContribution(d.data)).toFixed(2)}
                `)
        ...
        node.append("circle")
            ...
            .attr("fill", d => {
                ...
                if ((d.data.contributions || 0) > 1000)
    ...
    // Helper function to calculate team contribution recursively
    function calculateTeamContribution(node) {
        let total = node.contributions || 0;

        if (node.children && node.children.length > 0) {
            node.children.forEach(child => {
                total += calculateTeamContribution(child);
            });
        }
