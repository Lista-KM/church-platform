...
function displayReferralTreePreview($pdo, $referrerId) {
    ...
    $output = '<div class="tree-container" style="width: 100%; overflow-x: auto;">';
    $output .= '<svg id="referralTree" viewBox="0 0 800 400" xmlns="http://www.w3.org/2000/svg" style="width: 100%;">';
    ...
    // Draw the root node (referrer)
    $output .= '<g transform="translate(400, 40)">';
    $output .= '<rect class="node-bg-root" x="-70" y="-20" width="140" height="45" rx="8" />';
    $output .= '<circle class="avatar-bg-root" cx="-50" cy="0" r="15" />';
    $output .= '<text class="avatar-text" x="-50" y="0">' . substr($referrer['name'], 0, 1) . '</text>';
    $output .= '<text class="name-text" x="-25" y="0">' . htmlspecialchars($referrer['name']) . '</text>';
    $output .= '<text class="count-text" x="25" y="0">(' . $referralCount . ' referrals)</text>';
    $output .= '</g>';
    
    // If there are direct referrals, draw them
    if ($referralCount > 0) {
        // Calculate spacing for children
        $spacing = 800 / ($referralCount + 1);
        $startX = $spacing;
        
        // Draw connection lines from root to each child
        foreach ($referrals as $index => $referral) {
            $childX = $startX + ($index * $spacing);
            $output .= '<path class="tree-line" d="M400,65 C400,65 ' . $childX . ',140 ' . $childX . ',140" />';
        }
        ...
        foreach ($referrals as $index => $referral) {
            ...
            // Get child's referral count
            $stmt = $pdo->prepare("SELECT COUNT(*) FROM users WHERE referred_by = ?");
            $stmt->execute([$referral['id']]);
            $childReferralCount = $stmt->fetchColumn();
            
            // Draw the child node
            $output .= '<g transform="translate(' . $childX . ', 140)">';
            $output .= '<rect class="node-bg" x="-70" y="-20" width="140" height="45" rx="8" />';
            $output .= '<circle class="avatar-bg" cx="-50" cy="0" r="15" />';
            $output .= '<text class="avatar-text" x="-50" y="0">' . substr($referral['name'], 0, 1) . '</text>';
            $output .= '<text class="name-text" x="-25" y="0">' . htmlspecialchars($referral['name']) . '</text>';
            
            if ($childReferralCount > 0) {
                $output .= '<text class="count-text" x="25" y="0">(' . $childReferralCount . ' referrals)</text>';
                $output .= '<text class="register-text" x="0" y="40">Register to see complete tree</text>';
            }
...
// Add this near the end of your contribution.php file, replacing the existing referral tree preview section
?>